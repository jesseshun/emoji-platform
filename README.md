...
Phase 6D 在搜索能力之上，新增轻量、可解释、无 AI 的发现与推荐能力。所有推荐均基于**已发布的公开数据**，不依赖 Meilisearch 必须可用，也不引入个性化或用户画像。

### Discovery API

- `GET /api/v1/discovery/home?locale=zh`（或 `en`）：返回首页发现模块
  - `featuredEmojis`：精选 Emoji（按 `sortOrder` 升序，published）。
  - `featuredCategories`：全部分类（含 `emojiCount`，published）。
  - `featuredTopics`：最新专题（published）。
  - `latestArticles`：最新文章（published）。
  - 仅返回 published 内容，不含 draft / archived / admin 字段。

### Recommendation API

- `GET /api/v1/recommendations?entityType=emoji&slug=grinning-face&locale=zh&limit=8`
  - 参数：`entityType`（`emoji` / `category` / `topic` / `article`，非法值返回 **400**）、`slug`（必填，不存在返回 **404**）、`locale`（非法 / 缺失回退 `en`）、`limit`（默认 8，上限 **20**）。
  - 返回：`relatedEmojis` / `relatedCategories` / `relatedTopics` / `relatedArticles`（不适用的类型返回空数组）。
- 推荐规则（简单、可解释，无 AI）：
  - **emoji**：同分类 Emoji + 同专题 Emoji + 关键词相近 Emoji，排除自身，最多 8–12 个。
  - **category**：子分类 / 同级分类 + 关联专题 + 关键词相关文章。
  - **topic**：专题关联 Emoji + 共享 Emoji 的专题 + 关键词相关文章。
  - **article**：关键词相关文章 + 关键词相关专题 + 关键词相关 Emoji。
  - 关键词重叠打分（tokenize + 交集计数），只推荐 published 内容。

### 前台增强

- 首页（`/zh/`、`/en/`）改用 `DiscoverySection` 渲染发现模块（精选表情 / 分类 / 专题 / 最新文章）。
- 分类详情页新增「相关分类」与「相关文章」（来自 recommendations API）。
- 专题详情页新增「相关文章」（来自 recommendations API）。
- Emoji / 文章详情页的既有相关模块保持不变（已在 Phase 6C / 5C 实现）。

### 后台

- `/admin/discovery`：只读状态说明页（需登录，noindex），展示 discovery / recommendation API 状态、支持的实体类型、当前推荐规则、Meilisearch 是否作为辅助、fallback 状态。

### 不做 / 边界

- **不**使用 AI 推荐、**不**做个性化推荐、**不**建立用户画像。
- **不**接入第三方推荐服务、**不**批量生成 SEO 落地页、**不**把推荐结果页加入 sitemap。
- 推荐逻辑默认使用 **database**；Meilisearch 不可用时推荐仍可用（数据库兜底），公开页面不会因推荐失败而崩溃（前端静默降级）。
- 不破坏 Phase 6B Meilisearch provider / database fallback、Phase 6C Advanced Search UX、Phase 5 sitemap / robots / indexing，`/admin/*` 仍 `noindex`。

### 下一阶段

**Phase 6E - Search Analytics and Tuning**：在现有发现 / 推荐 / 搜索之上做搜索分析与调优（需经明确确认），保持现有 SEO 边界与动态渲染架构不变。

## 搜索分析与调优（Phase 6E）

Phase 6E 在搜索 / 索引 / 发现能力之上，新增**后台搜索分析与基础调优**。所有数据均为基于 `search_logs`（必要时参考 `copy_events`）的**只读聚合**，调优建议为**规则生成（不使用 AI）**，不自动改写任何内容，不引入机器学习或个性化。

### Search Analytics 范围

- 总搜索数、今日搜索数、零结果搜索数、零结果率。
- 按语言（zh / en / 其他）的搜索分布。
- 热门搜索词（Top 10）、热门零结果词（Top 10）。
- 最近搜索（最新 10 条，**不展示明文 IP / ipHash / User Agent**）。
- 按「关键词 + 语言」聚合的搜索词分析（次数、平均结果数、零结果次数、最近搜索时间）。
- 零结果搜索分析与**建议动作**（按语言给出补充别名 / 关键词 / shortcode 的建议）。
- Meilisearch provider 健康与 database fallback 状态。
- 规则化调优建议（见下）。

> 说明：`search_logs` 未记录每次请求命中的 provider（公开搜索在 Meilisearch 失败时会透明 fallback 到 database），因此「按 provider 的搜索占比」暂不支持；当前 provider 状态以 provider-health 为准。「搜索后复制」因搜索与复制事件间无可信会话 / 点击关联，亦暂不支持；原始复制分析见复制日志接口。

### Search Analytics API

全部需 admin 登录（未登录 401，权限不足 403），且不返回明文 IP / passwordHash / JWT_SECRET / Meilisearch API Key。

- `GET /api/v1/admin/search/analytics/overview`：聚合统计 + 规则化调优建议（totalSearches / todaySearches / zeroResultSearches / zeroResultRate / searchesByLocale / topQueries / topZeroResultQueries / recentSearches / providerStatus / tuningSuggestions 等）。
- `GET /api/v1/admin/search/analytics/queries`：搜索词分析（参数 `page` / `limit` / `locale` / `q` / `hasResults` / `dateFrom` / `dateTo`），返回按 query+locale 聚合的 `query / locale / count / avgResultCount / zeroResultCount / lastSearchedAt`。
- `GET /api/v1/admin/search/analytics/no-results`：零结果分析（参数 `page` / `limit` / `locale` / `dateFrom` / `dateTo`），返回 `query / locale / count / lastSearchedAt / suggestedAction`。
- `GET /api/v1/admin/search/analytics/provider-health`：当前 provider / fallback / Meilisearch 配置与可连接性 / 索引就绪 / 文档数 / fallback 建议。
- `POST /api/v1/admin/search/analytics/tuning/apply-settings`：保守应用 Phase 6B 既定 Meilisearch 设置（仅 `super_admin`）。未配置 Meilisearch 时返回 `available:false` 说明；成功时写 `audit_logs`（`search.tuning_settings_update`），不修改核心 ranking 逻辑，不自动改写内容。

### Search Analytics 后台页面

- `/admin/search/analytics`：分析视图（需登录，继承 admin 根布局的 `noindex,nofollow`）。展示概览卡片、语言分布、provider 占比说明、热门搜索词 / 零结果词、最近搜索（无明文 IP）、搜索词分析表、零结果分析表、Meilisearch 状态与 fallback 说明、规则化调优建议；`super_admin` 可见「应用保守调优设置」按钮。

### zero-result query 分析

- 聚合 `resultCount = 0` 的搜索记录，按「关键词 + 语言」分组统计次数与最近时间。
- 每条给出 `suggestedAction`：建议补充对应关键词的 Emoji alias / keywords 或 Article keyword；中文词补充中文关键词 / 别名，英文词补充英文 aliases / shortcode。

### tuningSuggestions 规则

规则生成（非 AI），示例：

1. 零结果率 ≥ 30% → 建议补充高频无结果词的 alias / keywords / Article keyword。
2. 某词多次零结果（≥5 次）→ 建议检查是否缺少对应 Emoji alias / Article keyword 或是否以 published 收录。
3. 中文零结果偏多 → 建议补充中文关键词 / 别名。
4. 英文零结果偏多 → 建议补充英文 aliases / shortcode。
5. Meilisearch 已配置但不可达 → 建议检查 Docker / env（含 `MEILISEARCH_API_KEY`），系统已自动 fallback database。
6. 索引未就绪 → 建议执行重建索引。

### Meilisearch settings 检查 / 应用

- 复用 Phase 6B 的 `SearchIndexService.configureIndexSettings()`，确认 `searchableAttributes` / `filterableAttributes`（含 type / locale / status）/ `sortableAttributes`（含 updatedAt / publishedAt）与既定一致。
- 仅做保守应用 / 核对，**不**根据日志自动改写 rankingRules、**不**自动把热门 query 变成内容、**不**自动生成 synonyms、**不**引入 AI、不破坏 fallback。

### 不做 / 边界

- **不**使用 AI 自动调优、**不**建立用户画像、**不**做个性化搜索、**不**引入机器学习。
- **不**批量生成 SEO 页面、**不**把搜索分析页加入 sitemap、**不**公开暴露 `search_logs`、不泄露明文 IP / API Key。
- 不破坏 `/api/v1/search`、Phase 6B Meilisearch provider / database fallback、Phase 6C Advanced Search UX、Phase 6D Discovery / Recommendation、Phase 5 sitemap / robots / indexing，`/admin/*` 仍 `noindex`。

### 下一阶段

**Phase 6F - Phase 6 Final Acceptance**：Phase 6 的最终验收与收尾（需经明确确认），回归 Phase 6A–6E 全部能力、搜索 / 索引 / 发现 / 推荐 / 分析，复查安全与 SEO 边界，确认可进入部署与长期维护（Phase 7）。
...
| GET | `/api/v1/discovery/home` | 首页发现模块（featuredEmojis / featuredCategories / featuredTopics / latestArticles，均 published，公开只读） |
| GET | `/api/v1/recommendations` | 内容推荐（参数 `entityType=emoji\|category\|topic\|article`、`slug`、`locale`、`limit`；返回 relatedEmojis / relatedCategories / relatedTopics / relatedArticles，公开只读） |
...
- **关键代码发现（Phase 7A 时）**：彼时 `apps/api/src/main.ts` 的 CORS **硬编码**为 `['http://localhost:3000','http://localhost:3001']`，不读取 `CORS_ORIGIN`；`web`/`admin` 的 `next.config` 未设 `basePath`。该问题已在 **Phase 7B** 解决——CORS 改为环境变量驱动（见下）。

### 下一阶段（历史）

**Phase 7B - Preview Environment, Secrets, and Config Hardening**：将 Phase 7A 规划落地为可运行的预览配置。✅ 已完成（见下）。

## Phase 7B - Preview Environment, Secrets, and Config Hardening（已完成）

Phase 7B 完成**预览环境配置加固**（仅代码配置层 + 文档 + 占位模板，**未真实部署、未连接真实服务器、未写真实密钥/IP**）。核心改动：

- **CORS 环境变量化**：`apps/api/src/main.ts` 不再硬编码 localhost，改为读取 `CORS_ORIGIN` / `ADMIN_ALLOWED_ORIGINS` / `WEB_BASE_URL` / `ADMIN_BASE_URL`（合并去重）。支持多 origin 逗号分隔、自动 trim、忽略空值；全部为空时回退到 `localhost:3000/3001`（本地开发开箱即用）。credentials=true 下白名单外来源一律拒绝，绝不使用 `*`。
- **Cookie 环境变量化**：`admin-auth.controller.ts` 的 `secure` / `domain` 改为读取 `COOKIE_SECURE` / `COOKIE_DOMAIN`。`COOKIE_SECURE` 显式优先，未设置时 production/preview 默认 `true`、本地默认 `false`；`COOKIE_DOMAIN` 为空时按当前 host 处理（IP+端口访问应留空）。
- **JWT_SECRET 消除弱默认**：`admin.module.ts` 原 `'change_me'` 弱默认已移除。缺失时 production/preview 启动失败（fail-fast），本地开发仅告警占位。
- **env 模板完善**：`.env.example` 增补 Phase 7B 变量；`.env.preview.example` 标注已实现；新增 `.env.production.example`（均为占位、可提交）。
- **Secrets 管理**：`.gitignore` 已覆盖 `.env` / `.env.local` / `.env.preview` / `.env.production` 及任意 `.env.*` 变体（不提交），例外保留三个 `.example` 模板（可提交）。GitHub PAT 仅一次性用于 push，不写入 remote/git config/任何文件。
- **Preview noindex / robots**：通过 `PREVIEW_NOINDEX=true` + Nginx `X-Robots-Tag: noindex` + `robots.txt` 全站 `Disallow` 双保险；不向搜索引擎提交 preview sitemap、不接国内 CDN、不做正式 SEO 提交。

详见 [docs/deployment/phase-7b-preview-config-hardening.md](./docs/deployment/phase-7b-preview-config-hardening.md)。

### 下一阶段

**Phase 7C - Preview Docker Deployment and Server Setup**：基于 Phase 7B 加固后的配置，完成单台服务器预览部署——`docker-compose.preview.yml` 真实化、Nginx preview 配置、HTTPS/证书策略、preview 库 seed/迁移、healthcheck 与备份演练。需经明确确认。

...
export default function AdminCategoriesPage() {
  ...
  const [meta, setMeta] = useState<AdminCategoryListMeta>({
    page: 1,
    limit: 20,
    total: 0,
    totalPages: 1,
    ...
    async (p: number) => {
      ...
      try {
        ...
        if (err instanceof AdminApiError) {
          setError(err.message || '加载分类列表失败');
        } else {
          setError('加载分类列表失败，请稍后重试');
        }
      } finally {
        setLoading(false);
      }
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">管理平台 Emoji 分类体系（支持父子层级）。</p>
        </div>
        {canManage && (
          <Link
      ...
      >
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1">父级</label>
          <select
            value={parentId}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b bg-gray-50">
              ...
              <th className="px-4 py-3">Emoji 数</th>
            ...
            {loading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 text-2xl">{item.iconEmoji || '—'}</td>
                  <td className="px-4 py-3 font-mono text-xs text-gray-600">{item.slug}</td>
                  <td className="px-4 py-3">{item.translations.zh.name || '—'}</td>
                  <td className="px-4 py-3">{item.translations.en.name || '—'}</td>
                  <td className="px-4 py-3">
                    {item.parent ? (
                      <span>
                        {item.parent.iconEmoji ? `${item.parent.iconEmoji} ` : ''}
                        {item.parent.name || item.parent.slug}
                      </span>
                    ) : (
                      '—'
                    )}
                  ...
                  <td className="px-4 py-3 text-gray-500">{item.emojiCount}</td>

...
export default function AdminTopicsPage() {
  ...
  const [meta, setMeta] = useState<AdminTopicListMeta>({
    page: 1,
    limit: 20,
    total: 0,
    totalPages: 1,
    ...
    async (p: number) => {
      ...
      try {
        ...
        if (err instanceof AdminApiError) {
          setError(err.message || '加载专题列表失败');
        } else {
          setError('加载专题列表失败，请稍后重试');
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-2xl font-bold mb-1">📝 专题管理</h1>
          <p className="text-gray-500 text-sm">管理平台 Emoji 专题（标题、内容、SEO 与 Emoji 关联）。</p>
        </div>
        {canManage && (
          <Link
            ...
            {loading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-500">{item.topicType || '—'}</td>
